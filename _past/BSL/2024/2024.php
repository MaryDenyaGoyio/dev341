...
$GamesQuery = "SELECT GP.PlayerID,
                ...
                FROM Games2024 G
                    RIGHT JOIN (SELECT Player1.PlayerID as PlayerID, Player1.Name as Name
                        FROM Games2024 G1 JOIN Players AS Player1 ON G1.Player1ID = Player1.PlayerID
                        UNION
                        SELECT Player2.PlayerID as PlayerID, Player2.Name as Name
                        FROM Games2024 G2 JOIN Players AS Player2 ON G2.Player2ID = Player2.PlayerID) AS GP
...
function updateRatings($conn) {
    ...
    while ($game = $result->fetch_assoc()) {
        ...
        $updateQuery = "UPDATE Games2024 SET Player1RatingBefore=?, Player1RatingAfter=?, Player2RatingBefore=?, Player2RatingAfter=?, RatingInc=?, Upset_yn=? WHERE GameID=?";
...
<head>
    ...
    <title> BCK Official 24 </title> <!-- 브라우져 탭 제목 -->
    <meta property="og:title" content="BCK Official 24"> <!-- 링크 공유시 페이지 이름 -->
...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        ...
        <label for="player1B_GO">B-GO:</label>
        <input type="text" id="B_GO" name="B_GO" maxlength="255"><br>
        <!-- 추가된 부분 끝 -->
